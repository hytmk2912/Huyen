...
def build_one(source: Source, config: dict[str, Any], dry_run: bool = False, loader: Any = None) -> dict[str, Any]:
    ...
    from local_ai.data.dedup import NearDedupIndex, NearDedupSettings
    text, near_dedup = NearDedupIndex(registry.db, NearDedupSettings.from_config(config)).filter_text(source.source_id, text)
    if not text.strip(): (paths["rejected"] / f"{source.source_id}.reason").write_text("near_duplicate"); return {"rejected": "near_duplicate", "near_dedup": near_dedup}
