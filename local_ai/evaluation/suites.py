...
def load_cases(path: str | Path) -> list[EvalCase]:
    cases = [EvalCase(**json.loads(line)) for line in Path(path).read_text(encoding="utf-8").splitlines() if line.strip()]
    ids = [case.id for case in cases]
    if len(ids) != len(set(ids)): raise ValueError(f"Mã câu hỏi bị trùng trong {path}")
    for case in cases:
        if case.scoring not in ("exact", "contains", "numeric"): raise ValueError(f"Câu {case.id}: không hỗ trợ cách chấm {case.scoring}")
    return cases
...
def parse_number(text: str) -> float | None:
    ...
    elif raw.count(",") > 1 or raw.count(".") > 1: raw = raw.replace(",", "").replace(".", "")
...
def score(case: EvalCase, output: str) -> bool:
    if case.scoring == "exact": return normalize(output) == normalize(case.expected)
    if case.scoring == "contains": return normalize(case.expected) in normalize(output)
...
def run_suites(cases: list[EvalCase], runner: Callable[[str], str]) -> dict[str, object]:
    suites: dict[str, dict[str, float]] = {}; failures: list[str] = []
    for case in cases:
        try: passed = score(case, runner(case.prompt))
        except Exception: passed = False
        bucket = suites.setdefault(case.suite, {"cases": 0, "correct": 0})
        bucket["cases"] += 1; bucket["correct"] += int(passed)
        if not passed: failures.append(case.id)
    for bucket in suites.values(): bucket["accuracy"] = bucket["correct"] / bucket["cases"]
    total = sum(bucket["cases"] for bucket in suites.values())
    return {"cases": total, "accuracy": sum(bucket["correct"] for bucket in suites.values()) / total if total else 0.0, "suites": dict(sorted(suites.items())), "failures": failures}


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="python -m local_ai.evaluation.suites", description="Chạy bộ đánh giá theo nhóm cho một model")
    ...
    from local_ai.models.smoke import missing_requirements
    cases = load_cases(args.cases); missing = missing_requirements()
    if missing: report = {"status": "skipped", "missing": missing, "cases": len(cases)}
    else:
        adapter = HuggingFaceModelAdapter(find_model_config(args.models_config, args.model))
        report = {"status": "completed", "model": args.model, **run_suites(cases, lambda prompt: adapter.generate([Message("user", prompt)]))}
    if args.report: Path(args.report).parent.mkdir(parents=True, exist_ok=True); Path(args.report).write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
