---
name: lam-moc
description: Làm mốc kế tiếp của nhiệm vụ 1 tuần trong TASKS.md. Dùng khi chủ repo gõ /lam-moc, bảo "làm tiếp nhiệm vụ tuần" hoặc "làm mốc tiếp theo", và khi lượt chạy tự động hằng đêm bắt đầu. Đọc memory.md và TASKS.md, làm đúng một mốc, chạy test/compileall/secret-scan, cập nhật memory.md rồi commit bằng tiếng Việt.
---
...
## 1. Đọc trạng thái
1. Nhánh làm việc là `claude/nhiem-vu-tuan`: remote có nhánh này thì checkout; chưa có (hoặc đã xoá sau khi gộp) thì tạo mới từ `main`. Không đẩy lên `main`.
...
5. Nếu mọi mốc đã xong hoặc bị chặn: ghi "XONG – chờ merge" vào `memory.md`, báo chủ repo rồi dừng. Không tự đặt thêm mốc mới.
...
2. Đẩy lên `claude/nhiem-vu-tuan` (`git push -u origin claude/nhiem-vu-tuan`), không đẩy thẳng lên `main`. Chưa có PR mở từ nhánh này vào `main` thì mở một PR (bản nháp); có rồi thì cập nhật checklist M1–M7 trong mô tả PR. Viết bằng tiếng Việt.
