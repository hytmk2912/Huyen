...
## Mốc đang làm
- Không có mốc dở. Mốc kế tiếp: **M14** (tổng kết: README, `docs/ARCHITECTURE.md`, lộ trình; `memory.md`: % tiến độ từng phần, việc chủ repo tự làm, đề xuất 7 mốc tuần 3).
...
| Lỗi | Nơi |
| --- | --- |
| Chưa chạy thật trên GPU: QLoRA, model ảnh + chữ, notebook Colab (M10, M11), agent với Ollama + qwen3:4b (M13); `target_modules: "all-linear"` có thể gắn LoRA vào phần xử lý ảnh. | `local_ai/models/adapters.py`, `local_ai/training/finetune.py` |
