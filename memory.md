...
- [ ] M15 Số đo thật trên Colab (đã chọn 25/9; **đang làm**: đủ số đo `smoke` và `light`, đã sửa ước tính; chờ số đo agent)
...
## Mốc đang làm
- **M15 — đang làm.** Đủ số đo thật của `smoke` và `light` trên T4 (`Hytmk2912/huyen-*-qlora`, `so_do/*.json`). Đã sửa `train_tflops` (5,2), VRAM QLoRA (`KBIT_OVERHEAD_GB`, PR #13); `token_overhead_s` giữ nguyên có căn cứ. Chỉ còn thiếu số đo agent.
  - 26/9 chủ repo trả lời: gộp PR #12 (đã gộp `8e44ac9`); đồng ý sửa công thức VRAM "tùy chỉnh sao cho hợp" (đã làm: `KBIT_OVERHEAD_GB` = 2,67 × √(tỷ tham số) cho model nén, 27B thành 34,6 GB, sửa test M2); việc 1 (chấm sau của `light`), 2 (`agent_colab`), 4 (tắt suy nghĩ của Qwen3 khi chấm) chủ repo tự kiểm tra rồi báo lại.
  - 26/9 (lượt 4): chủ repo chạy xong `light` (chấm sau 19/30, tool_use 7/8 → 3/8, bỏ `<think>`), nhờ cập nhật ước tính, gỡ torchao ở Bước 3, đề xuất cách giữ tool_use, gộp chung 1 PR (PR #13). Đã làm cả 3; thêm: lệnh train không ghi đè số đo khi chạy lại 0 bước.
  - Chờ chủ repo: chạy `agent_colab` (tiêu chí cuối của M15); chọn cách giữ tool_use (đề xuất trong README, hợp M19/M20); có tắt chế độ suy nghĩ khi chấm Qwen3 không.
  - Khi có báo cáo agent: điền cột Agent trong bảng README; khi đó M15 xong. Số đo train đúng của `light` ở `tests/fixtures/measurements/that_light_t4_2026-09-25.json` (trên Hub đã bị lần chạy 0 bước ghi đè).
...
10. Model chính 27B: ước tính mới khoảng 35 GB khi train QLoRA, cần GPU 40–48 GB (Colab free không đủ; chưa đo, đo ở M17). Chọn mốc tiếp theo trong M17–M21.
