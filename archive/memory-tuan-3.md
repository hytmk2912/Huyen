> Nhật ký chi tiết tuần 3: bản `memory.md` ngày 26/9/2026 chép nguyên văn trước lượt rà soát tuần 3 (từ lượt đó `memory.md` tối đa 5.000 ký tự). Các lượt sau ghi thêm chi tiết dài vào cuối file này.
...
| 26/9 | M19 (xong) | Chủ repo gọi `/lam-moc`. (1) Lệnh eval có `--no-thinking` (`enable_thinking=False` vào chat template; server nhận `chat_template_kwargs`); Bước 7 và 9 cùng dùng. (2) Bước 9 thêm `--hub-repo … --hub-path eval/sau --no-reuse`. (3) Thêm 8 câu tool_use (30 → 38 câu); `calibrate` ước tính thời gian chấm theo số câu của từng báo cáo để số đo thật 30 câu vẫn so đúng; thời gian trong README, TRAIN_COLAB tính lại (smoke 25 phút, light 130 phút). (4) Lỗi từ chối của TerminalTool kèm gợi ý (Việt + "Hint" tiếng Anh cho model); nhiệm vụ `thu-lai-khi-bi-tu-choi` (`wc -l` bị từ chối rồi `cat`), chỉ đạt khi có lệnh bị từ chối rồi gọi TerminalTool thành công. (5) `ModelConfig.temperature` (0 = greedy, truyền `do_sample=False` và bỏ temperature/top_p/top_k của Qwen), `seed`; server nhận `temperature` 0, `seed` 0; `settings` trong báo cáo có nguồn, revision, cách sinh chữ; `report.md` có dòng "Cài đặt". Test model tí hon có `generation_config` lấy mẫu: mặc định cũ ra khác nhau, nay 2 lần chấm giống hệt. (6) `answer_matches` so số theo giá trị (Decimal, bỏ dấu phân cách hàng nghìn). (7) Mã commit lấy từ API Hub (chỉ đọc thông tin; connector Hugging Face không trả SHA): 0.5B `7ae557604adf67be50417f59c2c2f167def9a775`, Qwen3-4B `1cfa9a7208912126459214e8b04321603b3df60c`, Coder-7B `c03e6d358207e414f1eca0bb1891e29f1db0e242`, Huihui 27B `739e3c5b89849f6c238ce1e5b70008612ae42cdd`. Test cũ khóa số (30 câu, 5 nhiệm vụ, payload server, khóa `settings`, Bước 9 không có `--hub-repo`) sửa theo yêu cầu mới của M19, không bỏ kiểm tra. `tests/test_m19_cham_cong_bang.py` 23 test; 294 test, check xanh. Tiếp theo: M20. |
